...
class PackIndexBuffer
{
	...
	public function new(shader:Shader, numVertex:Int,numIndices:Int,type:Int=0) 
	{
	    pipeline = shader;	
		switch (type)
		{
	   ...
	   this.capacity = numVertex;
	   vertexStrideSize =  inc *4; 
	   vertexBuffer = GL.createBuffer();
	   buffer = new Float32Array(numVertex * vertexStrideSize );
	...
	private function updtateVertex():Void
    {
		    dirt = false;
		    GL.bindBuffer(GL.ARRAY_BUFFER, vertexBuffer);
	        GL.bufferData(GL.ARRAY_BUFFER, buffer,mode);
	...
	private function updtateIndices():Void
    {
		    dirtIndices = false;
		     GL.bindBuffer(GL.ELEMENT_ARRAY_BUFFER, indexBuffer);
       GL.bufferData(GL.ELEMENT_ARRAY_BUFFER,  indices, GL.STATIC_DRAW);
	...
	public function addIndex(face:Int):Int
	{
		indices[indicescount] = face;
