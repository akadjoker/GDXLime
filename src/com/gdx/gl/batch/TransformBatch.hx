...
private var shaderProgram:GLProgram;
 ...
 public var SinAttribute :Int;
 public var CosAttribute :Int;
...
GL.shaderSource (vertexShader, Filter.textureMatrixVertexShader);
...
SinAttribute = GL.getAttribLocation (shaderProgram, "aSin");
CosAttribute = GL.getAttribLocation (shaderProgram, "aCos");
...
imageUniform = GL.getUniformLocation (shaderProgram, "uImage0");
	   ...
	   vertexStrideSize =  (2+2+1+2+2+2) *4 ; 
...
public  inline function Render(texture:Texture,m:Matrix2D,alpha:Float,blendMode:Int)
{
...
this.currentBatchSize++;
	   ...
	   public  function setProjMatrix(proj:Array<Float>)
   {
	   this.projs = new Float32Array(proj);

   }
    public  function setViewMatrix(view:Array<Float>)
   {
	   this.view = new Float32Array(view);
	...
	public  function Begin()
	{
		 ...
		 GL.uniformMatrix4fv(projectionMatrixUniform, false, projs);
		 GL.uniformMatrix4fv(modelViewMatrixUniform, false, view);
		...
		GL.vertexAttribPointer(vertexAttribute, 2, GL.FLOAT, false, vertexStrideSize, 0);
     	offset += 2;
        ...
        offset += 1;
	  	GL.enableVertexAttribArray (CosAttribute);
		GL.vertexAttribPointer(CosAttribute, 2, GL.FLOAT, false, vertexStrideSize, offset * 4);
        offset += 2;
		GL.enableVertexAttribArray (SinAttribute);
		GL.vertexAttribPointer(SinAttribute, 2, GL.FLOAT, false, vertexStrideSize, offset * 4);
        offset += 2;
