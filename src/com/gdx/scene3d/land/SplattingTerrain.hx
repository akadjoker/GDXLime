...
import com.gdx.gl.shaders.SplatTerrainShader;
 ...
 */
class SplattingTerrain extends SceneNode
{
	public var vertexBuffer:GLBuffer;
	public var indexBuffer:GLBuffer;
	public var pipeline:SplatTerrainShader;
	...
	public function new(HeightMap:String,highScale:Float,Precision:Float = 2,blockScale:Float = 2.0,scene:Scene,Parent:SceneNode = null , id:Int = 0, name:String="MultitexturedTerrain")  
	{
    	 super(scene, Parent, id, name);
	 	 pipeline = new SplatTerrainShader();
	...
	for (j in 0...height)
	{
		for (i in 0...width)
		{
            ...
            var T = ( j / (height - 1) );
			        ...
			        var xl = Math.round(i / (width-1  ) * img.width  )  ;
	...
	public function loadTextures(alpha:String,grass:String,stone:String,snow:String):Void
	{
		 texalpha = Gdx.Instance().getTexture(alpha, false);
		 texture0 = Gdx.Instance().getTexture(grass, true);
		 texture1 = Gdx.Instance().getTexture(stone, true);
		 texture2 = Gdx.Instance().getTexture(snow, true);
    ...
    pipeline.setTexture(texalpha, 0);	
