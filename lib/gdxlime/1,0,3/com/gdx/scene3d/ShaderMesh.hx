...
UpdateBoundingBox();
	 ...
	 */
	public  function loadH3DOptimize(filename:String,path:String,build:Bool=true,loadtexture:Bool=true):Void
	{
		...
		for (i in 0 ... numMaterials)
		{
			...
			var  texture:String = file.readUTFBytes(nameSize);
			if (Assets.exists(path + "/" + texture))
			{
			textures.set(i, Gdx.Instance().getTexture(path + "/" + texture, true));
			}
	    }
	    ...
	    for (i in 0 ... countMeshs)
		{
			...
			var surf:Surface = new Surface(null);
			...
			var numColors:Int = file.readInt();
			...
			for (x in 0...numVertices)
			{
				...
				if (nor.equalsToFloats(0.0, 0.0, 0.0))
				{
					nor.set(0.0, 1.0, 0.0);
				}
		...
		/// lets push this ;)
		//zort mesh by material and clone in big mesh
		...
		for (i in 0... surfs.length)
		{
			var surf1:Surface = surfs[i];
				...
				if (loadtexture)surf2.brush.setTexture(textures.get(surf2.materialIndex));
				if (build)surf2.UpdateVBO();
		...
		UpdateBoundingBox();
	 	...
	 	public  function loadH3D(filename:String,path:String,build:Bool=true,loadtexture:Bool=true):Void
	{
		var file:ByteArray =	Assets.getBytes(filename);
	    	file.endian = "littleEndian";
        if (file.bytesAvailable <= 0) return;
